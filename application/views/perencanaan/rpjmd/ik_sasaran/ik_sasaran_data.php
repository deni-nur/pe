...
<section class="content">
  ...
  <div class="box">
    <div class="box-header">
      <h3 class="box-title">Data Indikator Sasaran</h3>
      <div class="pull-right">
        <a href="<?=base_url('rpjmd/ik_sasaran/'.$this->uri->segment(3).'/add') ?>" class="btn btn-success btn-flat">
  ...
  <table class="table table-bordered table-rpjmdriped" id="table1">
    <thead>
      <tr>
        ...
        <tr>
          <?php $no=1; foreach ($ik_sasaran as $i => $data) { ?>
            <th colspan="8"><?=$data->nama_ik_sasaran ?></th>
      </tr>
    </thead>
    <tbody>
        ...
        <tr>
          ...
          <td rowspan="2">
            <a href="<?=site_url('rpjmd/ik_sasaran/'.$rpjmd->rpjmd_id.'/realisasi/'.$data->ik_sasaran_id) ?>" class="btn btn-success btn-xs">
              <i class="fa fa-pencil"></i> Realisasi
            </a>
            <a href="<?=site_url('rpjmd/ik_sasaran/'.$rpjmd->rpjmd_id.'/edit/'.$data->ik_sasaran_id) ?>" class="btn btn-warning btn-xs">
              <i class="fa fa-pencil"></i> Update
            </a>
           <!--  <a href="<?=site_url('ik_sasaran/del/'.$rpjmd->rpjmd_id.'/'.$data->ik_sasaran_id) ?>" onclick="return confirm('Yakin hapus data?')" class="btn btn-danger btn-xs">
    ...
    <tbody>
      <?php $no=1; foreach ($ik_sasaran_realisasi as $i => $data) { ?>
        <tr>
